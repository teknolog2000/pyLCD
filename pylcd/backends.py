    ...
    def __init__(self, display, pinmap, board=None, port=0):
        ...
        if board:
            ...
            try:
                ...
                raise IOError(
                    "Could not establish a connection to the K8055 board.")
...
class GPIOBackend:
    def __init__(self, display, pinmap):
        ...
        for pin, output in pinmap.iteritems():
            ...
            self.gpio.pinMode(output, self.gpio.PWM_OUTPUT if pin ==
                              'LED' and self.led_pwm else self.gpio.OUTPUT)
...
class ArduinoBackend:
    def __init__(self, display, pinmap, device="/dev/ttyACM0", pwm_outputs=[3, 5, 6, 9, 10, 11]):
        ...
        try:
            ...
            raise IOError(
                "Could not open the Arduino. Make sure you are running as root and are using the correct device name.")
    ...
    def write_nibble(self, nibble, data=True):
        self.serial.write("".join(chr(b) for b in [self.PIN_RS, int(data), self.PIN_D4, int(
            nibble[3]), self.PIN_D5, int(nibble[2]), self.PIN_D6, int(nibble[1]), self.PIN_D7, int(nibble[0])]))
    ...
    def set_brightness(self, level):
        ...
        if self.led_pwm:
            ...
            self.serial.write("".join(chr(b)
                                      for b in [self.PIN_LED, int(level > 0)]))
...
class DebugBackend:
    def __init__(self, display, pinmap, led_pwm=False, delay=0.01):
        ...
        self.pinmap = dict([(key, [_key for _key, value in self.output_states].index(
            key)) for key, value in pinmap.iteritems()])
        self.reverse_pinmap = dict([(value, key)
                                    for key, value in pinmap.iteritems()])
    ...
    def _update(self):
        if not self.printed:
            sys.stdout.write(
                " ".join([key.ljust(3) for key, value in self.output_states]) + "\n")
            self.printed = True
        sys.stdout.write(
            "\r" + " ".join(["#  " if value else "-  " for key, value in self.output_states]))
        sys.stdout.flush()
    ...
    def all_low(self):
        self.output_states = [[key, False]
                              for key, value in self.output_states]
        self._update()
